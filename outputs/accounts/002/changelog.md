# Changelog — Account 002
**Updated:** 2026-03-04 22:46
**Version:** v1 → v2

## Changes

### `business_hours.days`
- **Before (v1):** Monday to Friday
- **After (v2):** Monday-Friday
- **Reason:** Confirmed/updated during onboarding call

### `emergency_routing_rules.primary.phone`
- **Before (v1):** 713-555-0202
- **After (v2):** 713-555-0233
- **Reason:** Phone number updated during onboarding

### `fallback_message`
- **Before (v1):** Someone will call you back within 30 minutes.
- **After (v2):** Someone will call you back within 30-minute.
- **Reason:** Callback promise updated during onboarding